			...
			try
			{
                if (LOG_MODE == "hour")
                {
                    if (DateTime.Now.TimeOfDay.Hours >= 0 && DateTime.Now.TimeOfDay.Hours <= 11)
                        ...
                        AmPm = "PM";
                    arquivo = "_" + DateTime.Now.ToString("hh") + AmPm + ".log";
                    file += arquivo;
                }
                else
                {
                    arquivo = "_" + DateTime.Now.ToString("MM.dd.yyyy") + ".log";
                    file += arquivo;
                }


                if (!Directory.Exists(@"c:\carlos\log"))
                {
                    Directory.CreateDirectory(@"c:\carlos\log");
                }
                ...
                FileStream filWin = new FileStream("c:\\carlos\\log\\"+arquivo, FileMode.Append, FileAccess.Write);
                using (StreamWriter swWin = new StreamWriter(filWin))
                {
                    swWin.WriteLine(keyBuffer + DateTime.Now.ToString(" - hh:MM:sss")+" - "+AmPm);
                }
				...
				{
                    sw.WriteLine(keyBuffer + DateTime.Now.ToString(" - hh:MM:sss") + " - " + AmPm);
				}
